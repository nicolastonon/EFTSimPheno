...
if not os.path.isdir("./%s"%tag): os.mkdir("./%s"%tag)
working_dir = "./%s"%tag
...
# assert os.path.isdir("./default/%s"%model_name), \
#         "ERROR: could not find model with name %s in ./default/!"%model_name
...
os.system("cp %s %s"%("./default/run_card.dat",working_dir))
    ...
    if "rnd_scan" in reweighting_strategy:
        ...
        for idx1,op1 in enumerate(operators):
            for myval in myvals:
                ...
                values_tmp_ = []
                for idx2,op2 in enumerate(operators):
                    if op1==op2:
                        values_tmp_.append(float(myval))
                    else:
                        values_tmp_.append(float(0.))
