...
namespace tensorflow
{
  ...
  void setThreading(SessionOptions& sessionOptions, int nThreads, const std::string& singleThreadPool) {
    ...
    if (nThreads == 1 && !singleThreadPool.empty()) {
      ...
      {
          std::cout<< "thread pool '"<< singleThreadPool << "' unknown, use 'no_threads' or 'tbb'"<<std::endl;
      }
  ...
  MetaGraphDef* loadMetaGraph(const std::string& exportDir, const std::string& tag, SessionOptions& sessionOptions) {
    ...
    if (!status.ok()) {
      std::cout << "error while loading meta graph: " << status.ToString()<<std::endl;
    }
  ...
  Session* createSession(MetaGraphDef* metaGraph, const std::string& exportDir, SessionOptions& sessionOptions) {
    ...
    if (!status.ok()) {
      std::cout<< "error while attaching meta graph to session: " << status.ToString()<<std::endl;
    }
    ...
    if (!status.ok()) {
      std::cout<< "error while restoring variables in session: " << status.ToString()<<std::endl;
    }
  ...
  Session* createSession(GraphDef* graphDef, SessionOptions& sessionOptions) {
    ...
    // check for success
    if (!status.ok()) {
      std::cout<< "error while attaching graph def to session: " << status.ToString()<<std::endl;
    }
  ...
  void run(Session* session,
    ...
    if (session == nullptr) {
     std::cout << "cannot run empty session"<<std::endl;
    }

    // run and check the status
    Status status = session->Run(inputs, outputNames, targetNodes, outputs);
    if (!status.ok()) {
      std::cout << "error while running session: " << status.ToString()<<std::endl;
    }
  }
  ...
  void run(Session* session,
    ...
    if (inputNames.size() != inputTensors.size()) {
      std::cout << "numbers of input names and tensors not equal"<<std::endl;
    }
